...
const EditPeopleModal = (param: PeopleEditInt) => {
  const { isOpen, onClose, data } = param;
  const [errorMsg, setErrorMsg] = React.useState("");
  const [reqUpdate, resUpdate] = useUpdatePeopleMutation();
  ...
  const [profileSrc, setProfileSrc] = React.useState<string | null>(
    data.avatar
  );
  ...
  const formik = useFormik({
    initialValues: {
      ...
      age: data.age,
      gender: data.gender,
      category_id: Number(data?.category?.id) || 0,
      family_id: Number(data?.family?.id) || 0,
      avatar: data.avatar,
    },
  ...
  const onSubmit = (form: any) => {
    const formData = { ...form, avatar: profileSrc };
    reqUpdate({ data: formData, id: data.id });
  };
